...
**What we probe, and how it stays repeatable.** A frozen, versioned probe bank (`PROBE_BANK_VERSION`), 4 questions × market, written natively rather than machine-translated — the wording of a question moves the answer more than the model version does, so the wording is the thing we freeze. Each probe can be repeated 1–7 times in a run: the answers are stochastic — measured across four AI search engines, two simultaneous runs of the same prompt share only 46–48 % of the brands they name and 32–43 % of the sources, and the authors recommend at least 7 runs per prompt per day plus a 2–4-week rolling window ([Schulte, Bleeker & Kaufmann, Univ. of St. Gallen, April 2026](https://arxiv.org/abs/2604.07585)) — so the *stability* column tells the reader when a change is noise. Same bank, same brand, next month → comparable numbers and a delta.
...
- Concurrency 4 workers, retries with back-off. Measured on 9 Sept 2026 from the providers' own
  ...
  uses under 4 % of it — 4 workers is safe, and a run takes ~3.5 min (36 s per probe: 16 s answer + 20 s judge).
...
python test_app.py              # 234-assertion test battery — offline, no API key needed
...
provider side**: set a monthly budget on the OpenAI project and use a restricted key dedicated to this Space.
...
- **History is a file** (`runs/`). On the hosted Space it resets when the Space restarts; for monthly use, point `HISTORY_PATH` at persistent storage.
...
`examples/run_2026-09-09_loccitane.json` is the full export of the run behind the numbers quoted in the cover email: 3 markets × 4 probes × 2 assistants × 1 repetition = 24 answers, judged by `gpt-5.6-luna`, 96 API calls, $0.13. It contains every raw answer and every verdict, so each figure can be traced.
